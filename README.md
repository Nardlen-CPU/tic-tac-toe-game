...
- **Rematches:** after a win or draw, press the dedicated Rematch button in local two-player or Wi-Fi mode. Local rematches preserve scores and players. In Wi-Fi, both players must agree; the board stays intact until then. Duplicate votes are harmless and old-round requests are rejected.
- **Invited-room leagues:** after joining, either player can choose Room league and press Play league before the first move. The chosen league is shared by both players and determines which league receives the winner's trophy. After a completed round, a league selection applies to the next rematch and clears previous votes so both players agree again. League changes are blocked during a round. Wi-Fi leagues are player-versus-player matches without AI difficulty or league timers; local ladder unlocks do not restrict the shared room league selector.
